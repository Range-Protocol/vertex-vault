...
interface ISkateVertexVault is IERC20 {
    ...
    event MerkleRootsSet(bytes32 wmntMerkleRoot);
    ...
    function setMerkleRoots(bytes32 _wmntMerkleRoot) external;
    ...
    function wmntMerkleRoot() external view returns (bytes32);
    function wmntClaimedAmounts(address user) external view returns (uint256);
}
